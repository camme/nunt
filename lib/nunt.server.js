...
// read the nunt js file into memory and path it
...
function sendNuntJS(res) {
    ...
    if (!nuntJs) {
        ...
        var resource = nuntOptions.resource ? "resource: '" + nuntOptions.resource.replace(/^\//, '') + "', " : "";
        var clientConfig = "var base = {" + resource + " url: '" + nuntOptions.url + "', autoConnect: " + nuntOptions.autoConnect + "}; ";
        nuntJs = nuntJs.replace(findConfigRe, clientConfig);
...
function init(options) {
    ...
    if (options.resource) {
        socketIOOPtions.resource = options.resource;
    }

    var io = io.listen(server, socketIOOPtions);
    ...
    nunt.addGlobalListeners(function(event, client) {
        ...
        if (event.local !== true && event._local !== true) {
            ...
            if (event.sessionId) {
                ...
                var room = event._room;
                delete event._room;
                io.sockets.in(room).emit('nunt', event);
                //io.sockets.in(event._room).emit(event);
            }
            else {
